...
#   bash scripts/biz-deploy-basic.sh --status [target-path]   # read-only report
#   bash scripts/biz-deploy-basic.sh <target-path> --update    # no-overwrite + merge candidate list
...
# Re-sync the source pointer when --update AND the pre-existing .cursorrules carries
# REPLACE_BASICSOURCE or a stale path. Skipped when the file was written this run
# (existing_source empty): the pointer is already correct and a substitution would
# match the bare prefix, doubling the path.
if [[ "$MODE" == "update" ]] && [[ -n "$existing_source" ]] && [[ "$existing_source" != "$BIZ_ROOT" ]]; then
