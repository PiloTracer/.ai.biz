...
## Gaps found (follow-up status 2026-08-19)

- ~~`biz-cursorrules-verify.sh` has no `--self-test` and exits 1 when run against the source repo itself~~ — **resolved**: `--self-test` present (`scripts/biz-cursorrules-verify.sh:30-34`); self-hosted `source` layout detection makes a run against the framework repo PASS by design.
- ~~`templates/work/plans/NEXT.md.template` lacks the required `## Next action` heading~~ — **resolved**: heading present.
- ~~Stale artifacts in the repo: `tmp-fv-status.out`, `tmp/fv.log`~~ — **resolved**: removed.
- ~~`reasonix.toml` still allow-lists pre-rename paths~~ — **resolved**: allow-list reset to `[]`; the entries were exact-match one-off rename-era commands.
- `.ai` sibling row: the source `.cursorrules` registry already points at `../pilo.ai.logicbison`; the template keeps the legacy `../.ai` default with a note for family-named hosts.

## Checklist
- [x] `scripts/sister-discovery.sh` copied and `source`-able
- [x] `.cursorrules` registry: 7 rows + path resolution, paths resolve
- [x] Deploy fill wired (Layer 2 done; verified via `/tmp/smoke-biz` smoke)
- [x] Template registry (step 4) done
- [x] Verify commands pass (framework-verify, cursorrules-verify source+thin, deploy smoke)
- [x] Nothing committed/staged

## Next action
Next action: none — all steps applied and verified; commit when the owner asks.
